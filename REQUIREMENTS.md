...
7. **Geography, vulnerable groups, cross-border impact** — kept from current form. Geography options are EU, UK, USA, Australia and Global.
   - **Australia:** selecting it adds an "Australia AI practices" step with 12 Yes/No/Unsure questions based on the voluntary *Guidance for AI Adoption* (DISR / National AI Centre, 21 Oct 2025, successor to the 10-guardrail Voluntary AI Safety Standard). A deterministic engine (`lib/australia-alignment.ts`) decides per practice whether the system is aligned, partial or a gap, picks the foundations or implementation level, and gives an alignment verdict for both the Australian standard and the EU AI Act, with the reasoning, a cross-check of where they differ, and checklist items for every gap. "Unsure" never counts as aligned. The guidance is voluntary; the EU classification is not affected.
